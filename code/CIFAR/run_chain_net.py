...
model_config = {
	...
	'block_per_group': 1,
	'total_groups': 3,
	'downsample_type': 'avg_pool',  # conv, avg_pool, max_pool
	######################################################
...
if __name__ == '__main__':
	...
	if len(args.path) == 0:
		args.path = '../../Exp/TrainedNets/ChainNet' \
		            '/%s/L=%d_plane=%d' % \
		            (run_config.dataset, model_config['block_per_group'], model_config['start_planes'])
		args.path += '#valid=%s' % run_config.valid_size
		args.path += '#downsample=%s' % model_config['downsample_type']
		if model_config['use_identity']:
			args.path += '#identity'
		elif model_config['use_depth_sep_conv']:
	...
	model = ChainNet.set_standard_net(data_shape=data_shape, n_classes=n_classes, **model_config)
