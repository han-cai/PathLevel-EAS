...
from layers import LearnedLinear, LearnedGroupConv
...
parser.add_argument('--cell_stages', type=str, default='0-0-0-0-0')
parser.add_argument('--init_div_groups', action='store_true')
parser.add_argument('--batch_seq', type=int, default=1)
#####
parser.add_argument('--use_avg', action='store_true')
parser.add_argument('--bn_before_add', action='store_true')
#####
parser.add_argument('--path_drop', default=0.0, type=float)
parser.add_argument('--drop_only_add', action='store_true')
parser.add_argument('--use_zero_drop', action='store_true')
...
args.cell_stages = list(map(int, args.cell_stages.split('-')))
...
def main():
	...
	with open(os.path.join(args.savedir, args.filename), 'a') as fout:
		fout.write('FLOPs: %.2fM, Params: %.2fM' % (n_flops / 1e6, n_params / 1e6) + '\n')
		fout.write(str(args) + '\n')
	...
	if args.resume:
		...
		if checkpoint is not None:
			...
			if 'best_prec1' in checkpoint: best_prec1 = checkpoint['best_prec1']
			...
			if 'converted' in args.model:
				LearnedGroupConv.global_progress = 1.0
				LearnedLinear.global_progress = 1.0
			else:
				LearnedGroupConv.global_progress = float(args.start_epoch) / args.epochs
				LearnedLinear.global_progress = float(args.start_epoch) / args.epochs
	...
	if args.evaluate:
		...
		for epoch in range(args.start_epoch, args.epochs):
			### Train for one epoch
			tr_prec1, tr_prec5, loss, lr = train(train_loader, model, criterion, optimizer, epoch)  # train_loader
			...
			with open(os.path.join(args.savedir, 'valid.log'), 'a') as fout:
				fout.write('%d. Prec@1 %f \t Prec@5 %f\n' % (epoch, val_prec1, val_prec5))
				fout.flush()
...
def train(train_loader, model, criterion, optimizer, epoch):
	...
	for i, (input, target) in enumerate(train_loader):
		...
		if i % args.print_freq == 0 and i != 0:
			...
			print(log_str)
			with open(os.path.join(args.savedir, 'train.log'), 'a') as fout:
				fout.write('%d\t' % epoch + log_str + '\n')
				fout.flush()
